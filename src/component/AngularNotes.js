...
export default function AngularNotes({ notes, updateNoteImportance, editNote, deleteNote }) {
  ...
  const handleImportanceClick = (note) => {
    ...
    updateNoteImportance(note.id, newImportance, 'Angular');
  ...
  const handleEditSave = async () => {
    const updatedNote = {
      ...
      arrayName: 'Angular', // Adjust according to your note type
    };
    ...
    if (response.ok) {
      updateNoteImportance(updatedNote.id, IMP, 'Angular');
  ...
  const handleDeleteConfirm = async () => {
    ...
    if (response.ok) {
      updateNoteImportance(currentNote.id, IMP, 'Angular');
